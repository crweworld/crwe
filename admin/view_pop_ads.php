   ...
   if (isset($_GET['unpub']) and isset($_GET['check']) and ($_GET['check'])!="" )
{
	...
	{
		  $value; 
		 mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `pop_ads` SET `ad_status`= '0' WHERE ad_id IN ('" . implode("','",$value) . "')");	
...
if (isset($_GET['pub']) and isset($_GET['check']) and ($_GET['check'])!="" )
{
	...
	{
		  $value; 	
		mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `pop_ads` SET `ad_status`= '1' WHERE ad_id IN ('" . implode("','",$value) . "')");
	}	
...
if (isset($_GET['del_me'])and isset($_GET['check']) and ($_GET['check'])!="" )
{
	...
	{
		  $value; 	
		  $info = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pop_ads WHERE ad_id IN ('" . implode("','",$value) . "')")or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
		  while($results = mysqli_fetch_array($info))
		  {
		 	 $hash_key=$results['hash_key'];
			$ad_image=$results['ad_image'];
			 
			unlink("../uploads/pop/$hash_key/$ad_image");
			 unlink("../uploads/pop/$hash_key/thumb/$ad_image");
			 
			 rmdir("../uploads/pop/$hash_key/thumb");
			 	$del_fol= (scandir("../uploads/pop"));							
				foreach ( $del_fol as $key => $value )
				{
					if(count(scandir("../uploads/pop/$value")) == 2)
					 {
						rmdir("../uploads/pop/$value");
					 }
				}
	...
	foreach ( $array as $key => $value )
	{					
		mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM `pop_ads` WHERE ad_id IN ('" . implode("','",$value) . "')");
		mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM `pop_loc` WHERE ad_id IN ('" . implode("','",$value) . "')");
	}
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">View Popup Ad</li>
          </ol>
        </section>
            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title"><span class="active">View Popup Ad</span></h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                ...
                <div class="col-xs-6" style="position: absolute; left: 21%; z-index:10">
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    <?php  
					$vid_cat = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pop_ads ORDER BY ad_id DESC") or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
					...
					?>
                      <tr style="text-transform:capitalize;">
                        ...
                        <td><a href="edit_pop_ads.php?ad_id=<?php echo $ad_id ?>"><?php echo $ad_title ?></a></td>
                        ...
                        <td><span class="label <?php if($ad_status=="1"){echo "label-success"; $post_status="Active";} else{echo "label-warning"; $post_status="In Active";} ?>"><?php echo $post_status ?></span></td>
                          ...
                          <td><a href="pop_stats.php?ad_id=<?php echo $ad_id ?>"><i class="fa fa-line-chart"></i></a></td>
