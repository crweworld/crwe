...
if (isset($_GET['hot_vid']) and isset($_GET['check']) and ($_GET['check'])!="" )
{
	$array[] = $_GET['check'];
	$date = $_GET['date'];
	...
	{
		$value; 	
		mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `videos` SET `hot_video`= 'hot_video' WHERE vid_id IN ('" . implode("','",$value) . "')");	
	}	
	
	echo "<script>alert('Added to Hot Videos');
	window.location.href='?date=$date&find=$date';
	</script>";	
              ...
              <div class="box">
               ...
               <form action="#" method="get">
                ...
                <div class="col-xs-8" style="position: absolute; left: 15%; z-index:10">
                  ...
                  <div class="col-xs-3"><button type="submit" name="hot_vid" class="btn btn-block btn-info">Add to Hot Videos</button></div>
                  </div></div>
                  ...
                  <input type="text" name="date" style="visibility:hidden" value="<?php $date= $_GET['date']; echo $date?>">
					...
					{	
						...
						$cat_id = mysql_query("SELECT * FROM user where `id`='$user_id'") or die(mysql_error()); 
						while($cat_result = mysql_fetch_array($cat_id))
						{
						...
						$cat_id = mysql_query("SELECT * FROM admin where `id`='$admin_id'") or die(mysql_error()); 
						while($cat_result = mysql_fetch_array($cat_id))
						{
					...
					?>
                      ...
                      <td><input  class="checkbox1" type="checkbox" name="check[]" value="<?php echo $vid_id ?>"  /></td>
                        ...
                        <?php /*?><td><a href="edit_user.php?id=<?php echo $user_id ?>" target="_blank"><?php if($user_id!="0" and $admin_id=="0"){echo $username;} ?> </a><?php if($user_id=="0" and $admin_id!="0"){echo $adminname;} elseif($user_id=="0" and $admin_id=="0"){echo "Super Admin";}  ?></td><?php */?>
