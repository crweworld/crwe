   ...
   if (isset($_GET['unpub']) and isset($_GET['check']) and ($_GET['check'])!="" )
{
	...
	{ 
		 mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `stock` SET `post_status`= 'unpublish' WHERE post_id IN ('" . implode("','",$value) . "')");			
	}	
	echo "<script>alert('Unpublished');window.location.href='view_stock.php';</script>";		
}
...
if (isset($_GET['pub']) and isset($_GET['check']) and ($_GET['check'])!="" )
{
	...
	foreach ( $array as $key => $value )
	{	
		mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `stock` SET `post_status`= 'publish' WHERE post_id IN ('" . implode("','",$value) . "')");
	}	
	echo "<script>alert('Published');window.location.href='view_stock.php';</script>";	
}

if (isset($_GET['del']) and isset($_GET['check']) and ($_GET['check'])!="" )
{
	$array[] = $_GET['check'];	
	foreach ( $array as $key => $value )
	{	
		mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM `stock` WHERE post_id IN ('" . implode("','",$value) . "')");
	}	
	echo "<script>alert('Deleted');window.location.href='view_stock.php';</script>";	
}
            ...
            <div class="col-xs-12">
              <div class="box">
                ...
                <div class="col-xs-6" style="position: absolute; left: 21%; z-index:10">
                  ...
                  <div class="col-xs-4"><button type="submit" name="del" class="btn btn-block btn-danger">Delete</button></div>
                  </div>
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    <?php  
					$vid_cat = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM stock ORDER BY post_id DESC") or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
					...
					?>
                      <tr style="text-transform:capitalize;">
                        ...
                        <td><a href="edit_stock.php?stock_id=<?php echo $post_id ?>"><?php echo $post_title ?></a></td>
