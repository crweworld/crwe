                ...
                <div class="col-xs-8" style="position: absolute; left: 15%; z-index:10">
                 ...
                 <!-- <div class="col-xs-3"><button type="submit" name="slider" class="btn btn-block btn-info">Add to Slider</button></div>-->
                  ...
                  <input type="text" name="date" style="visibility:hidden" value="<?php $date= $_GET['date']; echo $date?>">
					...
					$posts = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM posts WHERE user_id={$_SESSION['pub_id']} and post_doc BETWEEN CAST('$date1' AS DATE) AND CAST('$date2' AS DATE) and (trend='trend_state' or trend='trend_country') ORDER BY post_id DESC") or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
					...
					?>
                      ...
                      <td><input class="checkbox1" type="checkbox" name="check[]" value="<?php echo $post_id ?>"  /></td>
                        ...
                        <td><?php echo $post_update ?></td>
                         <td><span><?php if($trend=="trend_state"){echo "State";} else {echo "Country";}?></span></td>
                        <td><?php if($trend=="trend_state"){echo $post_state;} else {echo $post_country;}?></td>
