...
if(isset($_POST['edit_user']))
{
	...
	$ref_sql="SELECT * FROM affi_user WHERE username='$referral'";
	...
	$ref_chk="SELECT * FROM affi_user WHERE username='$referral' and referral='$username1'";
	$result=mysql_query($ref_chk);
	$ref_chk=mysql_num_rows($result);
	...
	if($referral == $username1){ $err_sin .="<i class=\"icon fa fa-info\"></i> Error!!! Your Tracking Id and Referral Tracking Id is Same<br>";} 
	...
	if($err_sin=="")
	{
		...
		if($disabled=='1')
		{
			$sql="UPDATE `affi_user` SET `fname`='$fname',`lname`='$lname',`phone`='$phone',`referral`='$referral',`password`='$password',`type`='$type',`email`='$email',`web_url`='$web_url',`web_type`='$web_type',`web_visit`='$web_visit',`address`='$address',`post_city`='$post_city',`post_state`='$post_state',`post_country`='$post_country',`post_zipcode`='$post_zipcode' where `id`='{$_SESSION['affi_id']}'";	
		}
		else
		{
			$sql="UPDATE `affi_user` SET `fname`='$fname',`lname`='$lname',`phone`='$phone',`username`='$username',`referral`='$referral',`password`='$password',`type`='$type',`email`='$email',`web_url`='$web_url',`web_type`='$web_type',`web_visit`='$web_visit',`address`='$address',`post_city`='$post_city',`post_state`='$post_state',`post_country`='$post_country',`post_zipcode`='$post_zipcode' where `id`='{$_SESSION['affi_id']}'";	
		}
              ...
              <?php } ?>
                ...
                <form role="form" action="" method="post">                	
                  <div class="box-body">  
					 ...
					 <input class="form-control" name="username"   value="<?php if(isset($_SESSION['username'])){ echo $_SESSION['username']; } else{ echo ucwords(strtolower($results['username'])); }?>" placeholder="Enter Your Username / Tracking Id" type="text" required="required" <?php echo $disabled; ?>  > 
                     <?php if($disabled=='disabled'){ ?>
                     <input type="hidden" name="username" value="<?php echo ucwords(strtolower($results['username']));?>"  />
					  <?php } ?>
                    </div> 
                     <div class="form-group col-md-6">
                      <label>Referral Username / Referral Tracking Id</label> &nbsp; &nbsp;
					 <input class="form-control" name="referral"   value="<?php if(isset($_SESSION['referral'])){ echo $_SESSION['referral']; } else{ echo ucwords(strtolower($results['referral'])); }?>" placeholder="Enter Your Referral Username / Referral Tracking Id" type="text" >
                    </div>  
                  ...
                  <div class="form-group col-md-6">
                      ...
                      <textarea class="form-control" name="address" required="required"><?php if(isset($_SESSION['address'])){ echo $_SESSION['address']; } else{ echo $results['address'];} ?></textarea>
                    </div>
                    <div class="form-group col-md-6">
                    ...
                    <div class="form-group col-md-6">
                      <label>State <span class="red-star">*</span></label> &nbsp; &nbsp;
                       <input class="form-control" name="post_state"   value="<?php if(isset($_SESSION['post_state'])){ echo $_SESSION['post_state']; } else{ echo $results['post_state']; }?>" placeholder="Enter Your State" type="text" required="required">
                    </div>
                     <div class="form-group col-md-6">
                     ...
                     <div class="form-group col-md-6">
                      <label>Zipcode <span class="red-star">*</span></label> &nbsp; &nbsp;
                       <input class="form-control" name="post_zipcode"   value="<?php if(isset($_SESSION['post_zipcode'])){ echo $_SESSION['post_zipcode']; } else{ echo $results['post_zipcode'];} ?>" placeholder="Enter Your Zipcode" type="text" required="required">
                    </div>
                   
                  	<span class="h4 col-md-12"><i class="fa fa-sitemap"></i> <strong>AFFILIATE WEBSITE INFORMATION</strong></span>
                    <div class="form-group col-md-6">
                      <label>Website URL </label> &nbsp; &nbsp;
                       <input class="form-control" name="web_url"   value="<?php if(isset($_SESSION['web_url'])){ echo $_SESSION['web_url']; } else{ echo $results['web_url'];} ?>" placeholder="Enter Your First Name" type="url">
                    </div>
                    <div class="form-group col-md-6">
