...
if(isset($_POST['login']))
{
	...
	if($email=='' or $password==''){ $err1 = "<p class='err'>Please fill all the credentials</p>"; }
	elseif($count==1)
	{
		$active=mysqli_query($mysql_link, "select * from user where active='1' and (email='$email')");
...
else if(isset($_POST['register']))
{
	...
	if($count==0)
	{
		...
		if($fname==""){ $err2 .="Please enter your Frist Name. <br />";}
		if($lname==""){ $err2 .="Please enter your Last Name. <br />";}
		if($email==""){ $err2 .="Please enter your Email id. <br />";} 	
		elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)){ $err2 .="Please enter a valid email <br />";}
		if($password==""){ $err2 .="Please enter your Password. <br />";}
		if($cpassword==""){ $err2 .="Please enter your Confirm Password. <br />";}
		if($password!='' and $cpassword!='')
		{
			if($cpassword!=$password){ $err2 .="Password mismatch. <br />";}
		}
		elseif( strlen($password) <= 5){ $err2 .="Password should be greater than 5 characters  <br>";}
		
		if(!$captcha){$err2 .="Please check the captcha form. <br />";}
		
		if($err2=='')
		{	
			$insert_ok = mysqli_query($mysql_link, "INSERT INTO `user`(`fname`, `lname`, `doc`, `active`, `email`, `hash`, `password`,`group`) VALUES ('$fname', '$lname', '$doc','0', '$email', '$hash', '$password','$group')")or die(mysqli_error($mysql_link));
			if($insert_ok == 1)
			{				
				$subject = "CRWEWORLD Investment | Verification"; // Give the email a subject 
	...
	<div dir=\"ltr\"><br />
	  Thanks for signing up for CRWEWORLD Investment. Please take a moment to verify the email address associated with your CRWEWORLD Investment account by clicking the link below:</div>
	<div dir=\"ltr\"><br />
	</div>
	...
	<div dir=\"ltr\"><a href=\"http://investment.crweworld.com/privacy\" target=\"_blank\">http://investment.crweworld.com/privacy</a></div>
	...
	<div dir=\"ltr\">Please do not reply to this email; it was sent from an unmonitored email address. This message is a service email related to your use of CRWEWORLD investment.<br />
	</div>
	<div dir=\"ltr\"><br />
	</div>
	<div dir=\"ltr\">For general inquiries or to request support with your CRWEWORLD investment account: <a href=\"mailto:invest@crweworld.com\" target=\"_blank\">invest@crweworld.com</a></div>
	<div dir=\"ltr\"><br />
	</div>
	...
	else{ $err2 .="Email Id Already Exsist. <br />";}	
}
else if(isset($_POST['reset']))
{
	...
	if($count==1)
	{
		...
		window.location.href='';
			...
			$posts = mysqli_query($mysql_link, $sql) or die(mysqli_error($mysql_link)); 			
...
<div dir=\"ltr\"><br />
  Thanks for using CRWEWORLD investment. Please check the credentials for your email account:</div>
<div dir=\"ltr\"><br />
</div>
...
<pre>This email is part of a Closed-Loop Opt-In system and was sent to protect the privacy of the owner of this email address. Closed-Loop Opt-In confirmation guarantees that only the owner of an email address can subscribe themselves  to this mailing list.Furthermore, the following privacy policy is associated with this list:</pre>
	...
	{
		echo "<script>
		alert('We couldn\'t find an account using that email address. Try again or create a new account.');
		</script>";
	}
}
else if(isset($_POST['resend']))
{
	...
	$result=mysqli_query($mysql_link, $sql1);
	$count=mysqli_num_rows($result);
	$sql2="SELECT * FROM user WHERE email='$email' and active='1'";
	$result=mysqli_query($mysql_link, $sql2);
	...
	if($count==1)
	{
		...
		alert('An activation link is send to your email. Please check your spam folder in case you did not receive it');
			...
			$posts = mysqli_query($mysql_link, $sql1) or die(mysqli_error($mysql_link)); 			
			while($results = mysqli_fetch_array($posts))
			{	
			$fname=$results['fname'];
			$lname=$results['lname'];
...
<html>
<head>
<title>CRWEWORLD Investment |  Login</title>
<!-- custom-theme -->
...
<body class="bg agileinfo">
   <h1 class="agile_head text-center"> <a href="<?php echo $mainserver?>"><img src='/assets/images/logo.png' width="200"></a></h1>
            ...
            <div class="resp-tabs-container hor_1">
               <div class="crwe_agile_login">
                    ...
                    <form action="#" method="post" class="agile_form">
					 ...
					 <div class="login_crwels">
				        <a class="forgot" href="#forgot">Forgot Password</a><a class="resend" href="#active" style="float: right;"> Resend Activaction Link</a> <br />
					 </div>
                ...
                <div class="agile_its_registration">
                    ...
                    <form action="#" method="post" class="agile_form">					 
                      <p>First Name</p>
					  <input type="text" name="fname" value="<?php if(isset($_POST['fname'])){ echo $_POST['fname'];} ?>" required="required"/>
                      <p>Last Name</p>
					  <input type="text" name="lname" value="<?php if(isset($_POST['lname'])){ echo $_POST['lname'];} ?>" required="required"/>                      
					  <p>Email</p>
					  <input type="email" name="email"  value="<?php if(isset($_POST['email'])){ echo $_POST['email'];} ?>" required="required"/>
					  <p>Account Type</p>
					  ...
					  <input type="password" name="cpassword" required="required">
                        <div class="g-recaptcha" data-sitekey="6Le-zCcTAAAAAM_5Rm1De-l36uJXSkcu9NGpttVl"></div>         
         	  			<div class="check crwe_agileits">
                        	<label class="checkbox crwel"><input type="checkbox" name="checkbox" required="required"><i> </i>I accept the <a target="_blank" href="<?php echo $mainserver;?>/privacy">terms and conditions *</a></label>
						</div>
