...
$rss = Feed::loadRss('http://www.state.gov/rss/channels/sdo.xml');

foreach ($rss->item as $item)
{
	$post_image_loc="";
	$source_url= htmlSpecialChars($item->link); 
	$post_title= mysqli_real_escape_string($GLOBALS["___mysqli_ston"], htmlspecialchars_decode(htmlSpecialChars(str_replace('Speeches: ','',$item->title)))) ;
	$post_doc= date("Y-m-d", (int) $item->timestamp);
	$post_description=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $item->description);
	
$find = array();
$find[0]='/The Office of Website Management, Bureau of Public Affairs, manages this site as a portal for information from the U.S. State Department./';
$find[1]='/External links to other Internet sites should not be construed as an endorsement of the views or privacy policies contained therein./';
$replacements ="";
$post_description=preg_replace($find, $replacements, $post_description);	

	if (isset($item->{'enclosure'})){
		$post_image_loc= $item->enclosure['url'];
	 }
	 	
	$post_description .="<br> <br> Source: State.gov";
	$cat_id='3';
	$post_country='usa';
	$post_state='ny';
	$post_city='holtsville';
	$post_zipid='00501';
