...
$ban_geo1="SELECT * FROM banner_ads WHERE hash_key='$hash_key'";
...
if($count == 1)
{
	...
	/*gross click*/
	$ad_clicks++;									
	mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `banner_ads` SET `ad_clicks`='$ad_clicks' where hash_key='$hash_key' ")or die(mysqli_error($GLOBALS["___mysqli_ston"]));	
	...
	if($count == 0)
	{
	    ...
	    $ad_unq_clicks++;																	
		mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `banner_ads` SET `ad_unq_clicks`='$ad_unq_clicks' where hash_key='$hash_key' ")or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	}
