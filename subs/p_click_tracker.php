...
$pop_geo1="SELECT * FROM pop_loc WHERE pop_id='$pop_id'";
...
if($count == 1)
{
	...
	$ad_unq_clicks=$info['ad_unq_clicks'];
				...
				$info = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pop_ads where hash_key='$hash_key'") or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
	...
	/*gross click*/
	$ad_clicks++;									
	mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `pop_loc` SET `ad_clicks`='$ad_clicks' where pop_id='$pop_id' ")or die(mysqli_error($GLOBALS["___mysqli_ston"]));	
	...
	if($count == 0)
	{
	    ...
	    $ad_unq_clicks++;									
		mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE `pop_loc` SET `ad_unq_clicks`='$ad_unq_clicks' where pop_id='$pop_id' ")or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	}
